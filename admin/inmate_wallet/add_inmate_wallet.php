...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `inmate_wallet` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
    <form action="" id="visit-form">
        ...
        <div class="form-group">
            <label for="place" class="control-label">Earnings</label>
            <input type="number" name="amount" id="earnings" class="form-control  rounded-0" value="<?php echo isset($amount) ? $amount : ''; ?>"  required/>
        </div>
...
<script>
    $(document).ready(function(){
        ...
        $('#visit-form').submit(function(e){
            ...
            start_loader();
            $.ajax({
                url:_base_url_+"admin/model/add_inmate_wallet.php",
                ...
                success:function(resp){
                    if(typeof resp =='object' && resp.status == 'success'){
                        ...
                        uni_modal("<i class='fa fa-th-list'></i> inmate_wallet Details ","inmate_wallet/view_inmate_wallet.php?id="+resp.id)
